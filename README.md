...
### 1. 애플리케이션 구현하기
	1. 애플리케이션 구현
	- 지도에서 특정 지점을 선택하여 분석하는 기능을 구현하고자 mapedit 패키지를 사용한다.
	- mapedit는 지도의 특정 지점을 선택하거나 처리하는 selectModUI()와 callModule() 라이브러리를 제공한다.
	- selectModUI() : 지도 입력 모듈, 지도에서 특정 지점이 선택될 때 입력갑을 서버로 전달
	- callModule() : 입력 결과를 처리하여 다시 화면으로 전달하는 출력 모듈

### 2. 반응형 지도 애플리케이션 완성하기
	1. 반응식 설정하기
	- subset()으로 조건에 따라 아파트 실거래(apt_price) 자료를 필터링한다.
	- return()으로 필터링된 결과를 반환하고 apt_sel이라는 반응형 객체로 저장한다.
	- 또한 반으형 객체로 저장한 데이터는 다른 데이터와 구분하고자 뒤에 ()를 붙인다.
	- 슬라이더 입력 필터링 결과를 다른 곳에서 사용하려면 apt_sel()처럼 사용한다.
